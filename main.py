...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=form.email.data).first():
            flash('This login user with email is already exist,try instead to log in')
            return redirect(url_for('login'))

        hashed_passw = generate_password_hash(
...
def login():
    ...
    if login.validate_on_submit():
        ...
        try:
            ...
            flash('this email does not use,try again or you can regist')
            return render_template('login.html',form=login)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        return render_template("post.html",post_id=requested_post,comments=comments)
